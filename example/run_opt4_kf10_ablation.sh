...
# Three-repeat interleaved Opt4 v1 vs KF10 timing and acceptance selector.
...
ROOT_OUTPUT_DIR=${ROOT_OUTPUT_DIR:-"$REPO_ROOT/example/md_out/opt4_kf10_ablation_$(date '+%Y%m%d_%H%M%S')"}
...
case "$SCOPE" in
    model-only) BASE_FUSIONS=so2-epilogue ;;
    whole-step) BASE_FUSIONS=rmsnorm,so2-epilogue ;;
    *) echo "SCOPE must be model-only or whole-step" >&2; exit 2 ;;
esac
CANDIDATE_FUSIONS="$BASE_FUSIONS,so2-gate-bridge"
ROUND_DIR="$ROOT_OUTPUT_DIR/round_KF10"
...
if int(selected) == 0:
    accepted.append("so2-gate-bridge")
pathlib.Path(path).write_text(json.dumps({
    "scope": scope,
    "accepted_fusions": accepted,
    "focus_systems": focus.split(),
    "candidate_stage": "KF10",
    "policy": "energy/force errors are telemetry only",
}, indent=2) + "\n", encoding="utf-8")
PY

echo "KF10 selector exit: $selected"
echo "KF10 interleaved runner exit: $runner_code"
echo "KF10 selection: $selection"
echo "KF10 accepted fusions: $ROOT_OUTPUT_DIR/accepted_fusions.json"
echo "KF10 ablation output: $ROOT_OUTPUT_DIR"
exit "$selected"
