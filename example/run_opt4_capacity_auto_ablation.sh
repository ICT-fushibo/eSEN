...
# Interleaved Opt4 v2 whole-step uniform-vs-auto capacity ablation.
...
ROOT_OUTPUT_DIR=${ROOT_OUTPUT_DIR:-"$REPO_ROOT/example/md_out/opt4_capacity_auto_ablation_$(date '+%Y%m%d_%H%M%S')"}
...
ROUND_DIR="$ROOT_OUTPUT_DIR/round_CAP1AUTO"
...
selection="$ROOT_OUTPUT_DIR/CAP1AUTO_selection.json"
set +e
python "$REPO_ROOT/example/select_opt4_model_fusions.py" \
    --input-dir "$ROUND_DIR" --scope whole-step \
    --base-stage OPT4V2 --candidate-stage CAP1AUTO \
    --candidate-fusion auto-capacity --accepted-before "" \
...
python - "$ROOT_OUTPUT_DIR/accepted_capacity.json" "$selection" \
    "$NEIGHBOR_AUTO_MIN_REDUCTION" <<'PY'
import json
import pathlib
import sys

output, selection = map(pathlib.Path, sys.argv[1:3])
record = json.loads(selection.read_text(encoding="utf-8"))
output.write_text(json.dumps({
    "accepted": bool(record.get("accepted")),
    "base_policy": "uniform",
    "candidate_policy": "auto",
    "auto_minimum_reduction": float(sys.argv[3]),
...
echo "CAP1-auto selector exit: $selected"
echo "CAP1-auto selection: $selection"
echo "CAP1-auto output: $ROOT_OUTPUT_DIR"
exit "$selected"
