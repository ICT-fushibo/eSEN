...
# One-step compile/capture smoke for the KF11 Wigner-to-SO2 producer bridge.
...
OUTPUT_DIR=${OUTPUT_DIR:-"$REPO_ROOT/example/md_out/opt4_kf11_smoke_$(date '+%Y%m%d_%H%M%S')"}
...
echo "KF11 smoke: $status ($code)"
echo "KF11 smoke output: $OUTPUT_DIR"
[[ "$status" == completed ]]
