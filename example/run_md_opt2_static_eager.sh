...
# Opt2 ablation control: all fixed-capacity/padding/rotation adaptations are
# enabled, but model execution remains eager and no CUDA Graph is captured.

REPO_ROOT=$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)
CHECKPOINT=${CHECKPOINT:-"$REPO_ROOT/esen_30m_oam.pt"}
STRUCTURE_DIR=${STRUCTURE_DIR:-"$REPO_ROOT/../MatRIS-09bk/example/cif_file"}
RUN_ID=${RUN_ID:-"esen_opt2_static_eager_$(date '+%Y%m%d_%H%M%S')"}
...
if [[ "$REQUIRE_BASELINE_REFERENCE" == "1" && -z "$BASELINE_DIR" ]]; then
    echo "BASELINE_DIR is required for energy validation" >&2
    exit 2
fi
...

{
    ...
    echo "backend=opt2-static-eager"
    ...
    echo "static_setup_warmup=$CG_CAPTURE_WARMUP"
...
for system in "${systems[@]}"; do
    ...
    for temperature in "${temperatures[@]}"; do
        for repeat in $(seq 1 "$REPEATS"); do
            run_name="${system}_${temperature}K_${STEPS}step_esen_opt2_static_eager_r${repeat}"
            ...
            PYTHONPATH="$REPO_ROOT/src${PYTHONPATH:+:$PYTHONPATH}" \
                python -u "$REPO_ROOT/example/benchmark_md_opt2_static_eager.py" \
                    --structure "$structure" \
                    ...
                    --repeat "$repeat" \
                    --md-dtype "$MD_DTYPE" \
                    --probe-steps "$CG_PROBE_STEPS" \
                    --capacity-margin "$CG_CAPACITY_MARGIN" \
                    --edge-step "$CG_EDGE_STEP" \
                    --dummy-atoms "$CG_DUMMY_ATOMS" \
                    --setup-warmup "$CG_CAPTURE_WARMUP" \
                    --repeat-energy-atol "$CG_REPLAY_ENERGY_ATOL" \
                    --repeat-force-atol "$CG_REPLAY_FORCE_ATOL" \
                    "${reference_args[@]}" \
                    "${validation_args[@]}" \
                2>&1 | tee "$log_path"
            ...
            if [[ $exit_code -eq 0 && -f "$result_json" ]]; then
                ...
                echo "STATIC CAPACITY OVERFLOW: $run_name; continuing" >&2
            else
                status=error
                ((error_count += 1))
...
python "$REPO_ROOT/example/summarize_md_baselines.py" \
    ...
    --report-prefix opt2_static_eager_report
