#!/usr/bin/env python3
"""Poll idle GPUs for gated KF16 tiled Wigner-backward experiments."""
...
KF16_FUSION = "wigner-so2-tiled-backward"
KF16_MODEL_FUSIONS = f"{V4_MODEL_FUSIONS},{KF16_FUSION}"
KF16_WHOLE_FUSIONS = f"{V4_WHOLE_FUSIONS},{KF16_FUSION}"


def _selection_name(scope: str) -> str:
    return f"KF16_{scope.replace('-', '_')}_selection.json"


def _require_operator_gate() -> Path:
    value = os.environ.get("KF16_OPERATOR_GATE", "").strip()
    if not value:
        raise ValueError(
            "Set KF16_OPERATOR_GATE to an accepted KF16_operator_gate.json"
        )
    path = Path(value).resolve()
    try:
        result = json.loads(path.read_text(encoding="utf-8"))
    except (OSError, json.JSONDecodeError) as exc:
        raise ValueError(f"Cannot read KF16 operator gate: {path}") from exc
    if not (
        result.get("accepted") is True
        and result.get("correctness_ok") is True
        and result.get("graph_ok") is True
        and result.get("performance_ok") is True
        and result.get("candidate_fusion") == KF16_FUSION
    ):
        raise ValueError(f"KF16 operator gate was not accepted: {path}")
    return path


def _set_kf16_defaults() -> None:
    repo = Path(__file__).resolve().parents[1]
    operator_gate = _require_operator_gate()
    phase = os.environ.get("KF16_PHASE", "ablation").strip().lower()
    if phase not in {"smoke", "ablation", "formal"}:
        raise ValueError("KF16_PHASE must be smoke, ablation, or formal")
    defaults = {
        "smoke": {
    ...
    os.environ.setdefault(
        ...
        str(
            ...
            / "example"
            / "md_out"
            / f"opt4_kf16_{phase}_8gpu_{time.strftime('%Y%m%d_%H%M%S')}"
        ),
    )
    ...
    os.environ["MODEL_CANDIDATE_STAGE"] = "KF16_FP32"
    os.environ["MODEL_BASE_FUSIONS"] = V4_MODEL_FUSIONS
    os.environ["MODEL_CANDIDATE_FUSIONS"] = KF16_MODEL_FUSIONS
    ...
    os.environ["WHOLE_CANDIDATE_STAGE"] = "KF16_FP32"
    os.environ["WHOLE_BASE_FUSIONS"] = V4_WHOLE_FUSIONS
    os.environ["WHOLE_CANDIDATE_FUSIONS"] = KF16_WHOLE_FUSIONS
    ...
    os.environ.setdefault("RUN_KIND", f"opt4_v4_kf16_{phase}")
    os.environ.setdefault("STATUS_FILENAME", "kf16_status.tsv")
    os.environ["KF16_OPERATOR_GATE"] = str(operator_gate)

    if phase == "formal":
        selection_value = os.environ.get("KF16_SELECTION_DIR", "").strip()
        if not selection_value:
            raise ValueError(
                "KF16_PHASE=formal requires KF16_SELECTION_DIR from the "
        ...
        for scope in scopes:
            ...
            try:
                ...
                raise ValueError(f"Cannot read KF16 selection: {path}") from exc
            if not (
                result.get("accepted") is True
                ...
                raise ValueError(f"KF16 was not accepted for {scope}: {path}")
        os.environ["KF16_SELECTION_DIR"] = str(selection_dir)
