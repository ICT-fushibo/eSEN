...
: "${ROOT_OUTPUT_DIR:?Set ROOT_OUTPUT_DIR to the completed KF16 ablation}"
...
for scope in model-only whole-step; do
    ...
    output="$ROOT_OUTPUT_DIR/KF16_${scope//-/_}_selection.json"
    ...
    python "$REPO_ROOT/example/select_opt4_model_fusions.py" \
        ...
        --base-stage OPT4V4_FP32 --candidate-stage KF16_FP32 \
    ...
    set -e
    [[ $code -eq 0 ]] || overall=1
    echo "KF16 $scope selector exit=$code output=$output"
done
exit "$overall"
